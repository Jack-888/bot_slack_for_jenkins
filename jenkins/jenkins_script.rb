...
class ProjectJenkins
  ...
  def jenkins_status_project (way_project, project_name)
    ...
    if status_project == "running"
      "Jean: #{project_name} is building now."
    else
      time_built = time_built(way_project)
      "Jean: #{project_name} was built at #{time_built} with status #{status_project}."
    end
  end
